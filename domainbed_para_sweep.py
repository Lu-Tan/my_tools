...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        # command = ['python', '-m', 'domainbed.scripts.train_zin']
...
cmds = [job.command_str for job in jobs]  

world_size = args.gpus 
cuda_devices = [i for i in range(world_size)]

def main(rank):
    # 每个进程只在一张卡上运行
    putenv('CUDA_VISIBLE_DEVICES', str(cuda_devices[rank])) 
    for i,task in enumerate(cmds):
...
if "__main__" == __name__:
    ...
    pool.map(main, range(world_size))  # 并行执行函数
